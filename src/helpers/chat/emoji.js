 ...
 * @property {string} unicode list of code points, as hyphenated hex string (no joiners/not qualified)
 ...
 * @type {EmojiMap}
 */
export const emojiData = require('~/static/emoji/emoji.json');
 ...
 */
export const emojiByCanonicalShortname = buildMap();

/**
 * Maps _all_ shortnames (including aliases) to emoji. Useful for completions and lookups.
 * @type {{ [shortname : string] : Emoji }}
 */
export const emojiByAllShortnames = { ...emojiByCanonicalShortname };
Object.values(emojiData).forEach(emojiList => {
    emojiList.forEach(emoji => {
