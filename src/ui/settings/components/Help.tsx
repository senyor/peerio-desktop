...
export default class Help extends React.Component {
    ...
    copyLogs = () => {
        try {
            // `any`-cast needed since this is monkeypatched in src/helpers/console-history
            clipboard.writeText((console as any).history.toString());
            warnings.add('snackbar_logsCopied');
        } catch (e) {
            console.warn('Logs not copied!');
            if (isDevEnv) {
                console.warn(
                    'Running in dev environment! console.history is probably not available.'
                );
            }
        }
    };
