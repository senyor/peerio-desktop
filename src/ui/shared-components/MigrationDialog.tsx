...
const migrationInProgressMessages = [] as JSX.Element[];
...
for (let i = 1; i <= MIGRATION_MESSAGES_COUNT; i++) {
    ...
    if (i === 2) {
        ...
        migrationInProgressMessages.splice(
            random,
            0,
            <span>
                <T k={`title_migrationInProgressMessage{i}a` as any} />
                &nbsp;
                <EmojiImage emoji="muscle" />
                .&nbsp;
                <T k={`title_migrationInProgressMessage{i}b` as any} />
            </span>
        );
    } else {
        migrationInProgressMessages.splice(
            random,
            0,
            <T k={`title_migrationInProgressMessage${i}` as any} />
        );
    }
}
...
export default class MigrationDialog extends React.Component {
    ...
    render() {
        ...
        const migrationDialogActions = [
            {
                label: fileStore.migration.hasLegacySharedFiles
                    ? t('button_update')
                    : t('button_ok'),
        ...
        const textParser = {
            download: text => (
                <a className="clickable" onClick={this.downloadFile}>
                    {text}
                </a>
            )
        };
                ...
                {fileStore.migration.started || fileStore.migration.performedByAnotherClient ? (
                    ...
                    <div>
                        ...
                        {fileStore.migration.hasLegacySharedFiles ? (
                            <T k="title_upgradeFileSystemDescription3" tag="p">
                                {textParser}
                            </T>
                        ) : null}
                    </div>
