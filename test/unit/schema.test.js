...
const hardBreakOnly = chatSchema.node('doc', null,
    chatSchema.node('paragraph', null,
        chatSchema.node('hard_break')
    )
);

const paragraphWithText = chatSchema.node('doc', null,
    chatSchema.node('paragraph', null,
        chatSchema.text('some sample text')
    )
);

const paragraphWithWhitespace = chatSchema.node('doc', null,
    chatSchema.node('paragraph', null,
        chatSchema.text('     ')
    )
);

const paragraphWithEmoji = chatSchema.node('doc', null,
    chatSchema.node('paragraph', null,
...
describe('chat schema', () => {
    [
        ...
        { doc: paragraphWithEmoji, name: 'paragraph with emoji', shouldBeWhitespaceOnly: false }
    ].forEach(testCase => {
